...
export const operatorHelpMap: Record<OperatorType, OperatorHelpEntry> = {
  ...
  [OperatorType.ConditionalAssign]: {
    ...
    configs: [
      { name: '条件规则列表', desc: '每条规则包含：列名、运算符、比较值、满足时的结果值。从上到下依次匹配，命中第一条即停止' },
      { name: '默认值', desc: '所有条件都不满足时使用的兜底值' },
      { name: '输出列名', desc: '新增的结果列名' },
    ],
    tips: [
      '规则按顺序匹配，命中第一条即停止（类似 if / else if / else）',
      '典型场景：惩罚机制（金币 < 100 → 扣 50）、奖励标记（出勤 >= 25 天 → 奖 200）',
      '与阶梯规则的区别：阶梯只处理数值区间映射，条件赋值支持任意列、任意比较运算',
      '不同规则可以判断不同的列（如第一条判金币，第二条判时长）',
    ],
  },
  ...
  [OperatorType.GroupBy]: {
    ...
    configs: [
      ...
      { name: '聚合规则', desc: '每条规则包含：源列名、聚合函数（sum/count/avg/min/max）、输出列名' },
    ],
    tips: [
